...
Omni.getnewaddress = function(account, cb) {
    if (account == null || account == undefined || account === 0) {
...
Omni.getbalance = function(account, confirmations, cb) {
    client.cmd('getbalance', account, 31, function(err, balance, resHeaders) {
        if (err) {
            console.log(err);
            return cb(null, err);
        }
...
Omni.getwalletaddressbalances = function(includewatchonly, cb) {
    client.cmd('omni_getwalletaddressbalances', includewatchonly, function(err, data) {
        if (err) {
            console.log(err);
            return cb(null, err);
        }
...
Omni.listtransactions = function(txid, count, skip, startblock, endblock, cb) {
    ...
    client.cmd('omni_listtransactions', txid, count, skip, startblock, endblock, function(
        err,
        data,
        resHeaders
    ) {
...
Omni.getdivisibleproperty = function(propertyid) {
    client.cmd('omni_getproperty', 'result', 'divisible', propertyid, function(
        err,
        data,
        resHeaders
    ) {
...
Omni.gettradehistoryaddress = function(address, trades, propertyfilter, cb) {
    if (propertyfilter == null) {
        client.cmd('omni_gettradehistoryforaddress', address, trades, function(
            err,
            data,
            resHeaders
        ) {
        ...
        client.cmd('omni_gettradehistoryforaddress', address, trades, propertyfilter, function(
            err,
            data,
            resHeaders
        ) {
...
Omni.senddexsell = function(address, id, amount1, amount2, paymentwindow, fee, action, cb) {
    //action = (1 for new offers, 2 to update, 3 to cancel)
    client.cmd(
        'omni_senddexsell',
        address,
        id,
...
Omni.senddexaccept = function(address, address2, id, amount, cb) {
    //action = (1 for new offers, 2 to update, 3 to cancel)
    client.cmd('omni_senddexsell', address, address2, id, amount, false, function(
        err,
        data,
        resHeaders
    ) {
...
Omni.sendcanceltradesbyprice = function(address, id1, amount1, id2, amount2, cb) {
    client.cmd('omni_sendcanceltradesbyprice', address, id1, amount1, id2, amount2, function(
        err,
        data,
        resHeaders
    ) {
...
Omni.sendchangeissuer = function(address1, address2, propertyid, cb) {
    client.cmd('omni_sendchangeissuer', address1, address2, propertyid, function(
        err,
        data,
        resHeaders
    ) {
...
Omni.sendtrade = function(address, id1, amount, id2, amount2, cb) {
    client.cmd('omni_sendtrade', address, id1, amount.toString(), id2, amount2.toString(), function(
        err,
        data,
        resHeaders
    ) {
...
Omni.sendgrant = function(address1, address2, id, amount, note, cb) {
    client.cmd('omni_sendgrant', address1, address2, id, amount, note, function(
        err,
        data,
        resHeaders
    ) {
